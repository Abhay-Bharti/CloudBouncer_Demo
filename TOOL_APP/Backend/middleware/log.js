...
const LogPath = "../../";
const logDirectory = path.join(LogPath, "logs");
...
const blockIplogFilePath = path.join(logDirectory, "blocked_ips.json");
if (!jsonLogFilePath) {
  fs.writeFileSync(jsonLogFilePath);
}
if (!fs.existsSync(blockIplogFilePath)) {
  fs.writeFileSync(blockIplogFilePath, JSON.stringify([]));
}
